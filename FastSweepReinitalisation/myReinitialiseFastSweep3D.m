function levelSet = myReinitialiseFastSweep3D(levelSet, h, maxSweeps)
%{
...
xLoop = [1 nx; 1 nx; nx 1; nx 1; 1 nx; 1 nx; nx 1; nx 1];
yLoop = [1 ny; ny 1; ny 1; 1 ny; 1 ny; ny 1; ny 1; 1 ny];
zLoop = [1 nz; 1 nz; 1 nz; 1 nz; nz 1; nz 1; nz 1; nz 1];
...
while (changed == true) && (sweepNr < maxSweeps)
    ...
    for i = xLoop(d,1): sign(xLoop(d,2)-xLoop(d,1)) :xLoop(d,2)
        for j = yLoop(d,1): sign(yLoop(d,2)-yLoop(d,1)) :yLoop(d,2)
            for k = zLoop(d,1): sign(zLoop(d,2)-zLoop(d,1)) :zLoop(d,2)

    			[levelSet, changed] = ...
                    upwindLevelUpdate(levelSet,h,setQ,changed,i,j,k);
...
if ~setQ(i,j,k)
    ...
    if i == 1
        levXpos = levelSet(i+1,j,k);
        levXneg = levXpos;
    elseif i == sz(1)
        levXneg = levelSet(i-1,j,k);
        levXpos = levXneg;
    else
        levXpos = levelSet(i+1,j,k);
        levXneg = levelSet(i-1,j,k);
    ...
    if j == 1
        levYpos = levelSet(i,j+1,k);
        levYneg = levYpos;
    elseif j == sz(2)
        levYneg = levelSet(i,j-1,k);
        levYpos = levYneg;
    else
        levYpos = levelSet(i,j+1,k);
        levYneg = levelSet(i,j-1,k);
   ...
   if k == 1
        levZpos = levelSet(i,j,k+1);
        levZneg = levZpos;
    elseif k == sz(3)
        levZneg = levelSet(i,j,k-1);
        levZpos = levZneg;
    else
        levZpos = levelSet(i,j,k+1);
        levZneg = levelSet(i,j,k-1);
    ...
    if (lev > 0)
        ...
        lev3 = min(levZpos,levZneg);
        levHat = findRootEikonalND([lev1,lev2,lev3],h,true);

        % Update level set if distance value is lower
        if (levHat < lev)
    		levelSet(i,j,k) = levHat;
    		changed = true;
        end
    ...
    elseif (lev < 0)
        ...
        lev3 = max(levZpos,levZneg);
        levHat = findRootEikonalND([lev1,lev2,lev3],h,false);
